...
const PublishForm = () => {
    ...
    const publishBlog = (e) => {
        ...
        if (!des.length || !des.length > characterLimit) {
            return toast.error(`Write the description within ${characterLimit} characters to publish`)
        }
        if (!tags.length) {
            return toast.error("Enter atleast 1 tag to help us rank your blog")
        }
        let loadingToast = toast.loading("✍️ Publishing...");
        e.target.classList.add('disable');

        let blogObj = {
            title, banner, tags, content, des, draft: false
        }
    ...
    return (
        <AnimationWrapper>
            <section className="w-screen min-h-screen grid items-center lg:grid-cols-2 py-16 lg:gap-4">
                ...
                <button className="w-12 h-12 absolute right-[5vw] z-19 top-[5%] lg:top-[10%]"
                    onClick={handleCloseEvent}
                >
                    <i className="fi fi-br-cross"></i>
                </button>

                <div className="max-w-[550px] center mb-8">
