...
const NotificationCard = ( { data,index,notificationState, }) => {
    ...
    return (
        <div className={"p-6 border-b border-grey border-l-black  " + (!seen ? "border-l-2":  "") }>
            <div className="flex gap-5 mb-3 ">
                ...
                <div className="w-full ">
                    <h1 className="font-medium text-xl text-dark-grey">
                        ...
                        <Link to={`/user/${username}`} className="mx-1 text-block underline">@{username}</Link>
                    ...
                    </h1>
                    {
                        ...
                        :
                        <Link to={`/blog/${blog_id}`} className="font-medium text-dark-grey  line-clamp-1 hover:underline">
                            {`"${title}"`}
                        </Link>
                    }
            ...
            <div className="ml-14 pl-4 text-dark-grey mt-3 flex gap-8">
            ...
            }
            {
                ...
                <div className="ml-20 p-5 bg-grey mt-5 rounded-md">
                    ...
                    <div>
                        <h1 className="font-medium text-xl text-dark-grey">
                            <Link to={`/user/${author_username}`} className="underline mx-1 text-black">@{author_username}</Link>
                            <span className="font-normal">replied to</span>

                            <Link to={`/user/${username}`} className="underline mx-1 text-black">@{username}</Link>
                        </h1>
                    </div>
                    </div>
                    <p className="font-gelasio text-xl ml-14 my-2 ">{reply.comment}</p>
