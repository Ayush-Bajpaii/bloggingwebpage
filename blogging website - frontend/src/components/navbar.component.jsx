        ...
        >
          <div className="relative w-full md:w-[300px]">
            <input
              type="text"
              placeholder="Search"
              className="w-full bg-gray-100 p-4 pl-12 pr-12 rounded-full placeholder:text-dark-grey min-h-12"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              onKeyDown={handleSearch}
            />
            <i
              className="fi fi-rr-search absolute right-4 top-1/2 -translate-y-1/2 text-xl text-gray-500 cursor-pointer hover:text-gray-700"
              ...
              <Link to="/dashboard/notifications">
                <button className="w-12 h-12 rounded-full bg-grey relative hover:bg-black/10">
                  ...
                  {new_notification_available ? (
                    <span className="bg-red w-3 h-3 rounded-full absolute z-10 top-1 right-2"></span>
                  ) : (
                    " "
                  )}
                </button>
