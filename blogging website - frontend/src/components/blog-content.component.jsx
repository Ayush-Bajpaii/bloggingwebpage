const Img = ({ url , caption}) =>{
    return (
        <div>
            <img src={url} />
            { caption.length ? <p className="w-full text-center my-3 md:mb-12 text-dark-grey text-base ">{caption}</p> : " " }
        </div>
    )
}
...
const Quote = ({ quote , caption }) => {
    return (
        <div className="bg-purple/10 p-3 pl-5 border-l-4 border-purple">
            ...
            { caption.length ? <p className="w-full text-center  text-purple text-base ">{caption}</p> : " " }
        </div>
    )
}
